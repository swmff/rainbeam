...
impl Database {
    ...
    /// Get a page from a database result
    pub async fn gimme_page(&self, res: HashMap<String, String>) -> Result<Page> {
        Ok(Page {
            id: res.get("id").unwrap().to_string(),
    ...
    pub async fn get_page(&self, id: String) -> Result<Page> {
        ...
        match self
            .base
            .cachedb
            .get(format!("rbeam.app.page:{}", id))
            .await
        {
            Some(c) => {
                match serde_json::from_str::<HashMap<String, String>>(c.as_str()) {
                    Ok(res) => {
                        return Ok(match self.gimme_page(res).await {
                            Ok(r) => r,
                            Err(e) => return Err(e),
                        })
                    }
                    Err(_) => {
                        ...
                        self.base
                            .cachedb
                            .remove(format!("rbeam.app.page:{}", id))
                            .await
                    }
                };
            }
        ...
        let query: String = if (self.base.db.r#type == "sqlite") | (self.base.db.r#type == "mysql")
        {
            "SELECT * FROM \"xpages\" WHERE \"id\" LIKE ?"
        } else {
            "SELECT * FROM \"xpages\" WHERE \"id\" LIKE $1"
        }
        .to_string();
        ...
        // return
        let page = match self.gimme_page(res).await {
            Ok(r) => r,
            Err(e) => return Err(e),
        };
        ...
        self.base
            .cachedb
            .set(
                format!("rbeam.app.page:{}", id),
                serde_json::to_string::<Page>(&page).unwrap(),
            )
            .await;
    ...
    /// # Arguments
    /// * `id`
    pub async fn get_page_by_owner_slug(&self, owner: String, slug: String) -> Result<Page> {
        // check in cache
        match self
            .base
            .cachedb
            .get(format!("rbeam.app.page:{}:{}", owner, slug))
            .await
        {
            Some(c) => {
                match serde_json::from_str::<HashMap<String, String>>(c.as_str()) {
                    ...
                    Err(_) => {
                        ...
                        self.base
                            .cachedb
                            .remove(format!("rbeam.app.page:{}:{}", owner, slug))
                            .await
                    }
                };
            }
        ...
        let query: String = if (self.base.db.r#type == "sqlite") | (self.base.db.r#type == "mysql")
        {
            "SELECT * FROM \"xpages\" WHERE \"owner\" = ? AND \"slug\" = ?"
        } else {
            "SELECT * FROM \"xpages\" WHERE \"owner\" = ? AND \"slug\" = ?"
        }
        .to_string();
        ...
        self.base
            .cachedb
            .set(
                format!("rbeam.app.page:{}:{}", owner, slug),
                serde_json::to_string::<Page>(&page).unwrap(),
            )
            .await;
    ...
    /// # Arguments
    /// * `author`
    pub async fn get_pages_by_author_paginated(
        &self,
        author: String,
        page: i32,
    ) -> Result<Vec<Page>> {
        ...
        let query: String = if (self.base.db.r#type == "sqlite") | (self.base.db.r#type == "mysql")
        {
            format!("SELECT * FROM \"xpages\" WHERE \"owner\" = ? ORDER BY \"edited\" DESC LIMIT 25 OFFSET {}", page * 25)
        } else {
            format!("SELECT * FROM \"xpages\" WHERE \"owner\" = $1 ORDER BY \"edited\" DESC LIMIT 25 OFFSET {}", page * 25)
        };
    ...
    /// Create a new page
    ///
    /// Pages are a tiered feature. Users can only create pages if they are of the correct account tier.
    ///
    /// # Arguments
    /// * `props` - [`PageCreate`]
    ...
    pub async fn create_page(&self, props: PageCreate, author: String) -> Result<Page> {
        ...
        if author.tier < self.server_options.tiers.pages {
            return Err(DatabaseError::NotAllowed);
        }
        ...
        let query: String = if (self.base.db.r#type == "sqlite") | (self.base.db.r#type == "mysql")
        {
            "INSERT INTO \"xpages\" VALUES (?, ?, ?, ?, ?, ?, ?)"
        } else {
            "INSERT INTO \"xpages\" VALEUS ($1, $2, $3, $4, $5, $6, $7)"
        }
        .to_string();
        ...
        match sqlquery(&query)
            .bind::<&String>(&page.id)
            .bind::<&String>(&page.slug)
            .bind::<&String>(&page.owner)
            .bind::<&String>(&page.published.to_string())
            .bind::<&String>(&page.edited.to_string())
            .bind::<&String>(&page.content)
            .bind::<&String>(&match serde_json::to_string(&page.context) {
                Ok(s) => s,
                Err(_) => return Err(DatabaseError::ValueError),
            })
            .execute(c)
    ...
    pub async fn update_page_content(
        ...
        // make sure the response exists
        let page = match self.get_page(id.clone()).await {
            Ok(q) => q,
            Err(e) => return Err(e),
        };

        // check content length
        if content.len() > (64 * 4096) {
            return Err(DatabaseError::ContentTooLong);
        }
        ...
        let query: String = if (self.base.db.r#type == "sqlite") | (self.base.db.r#type == "mysql")
        {
            "UPDATE \"xpages\" SET \"content\" = ?, \"edited\" = ? WHERE \"id\" = ?"
        } else {
            "UPDATE \"xpages\" SET (\"content\", \"edited\") = ($1, $2) WHERE \"id\" = $3"
        }
        .to_string();
            ...
            .await
        {
            Ok(_) => {
                ...
                self.base
                    .cachedb
                    .remove(format!("rbeam.app.page:{}:{}", page.owner, page.slug))
                    .await;
    ...
    /// * `user` - the user doing this
    pub async fn delete_page(&self, id: String, user: Profile) -> Result<()> {
        ...
        let query: String = if (self.base.db.r#type == "sqlite") | (self.base.db.r#type == "mysql")
        {
            "DELETE FROM \"xpages\" WHERE \"id\" = ?"
        } else {
            "DELETE FROM \"xpages\" WHERE \"id\" = $1"
        }
        .to_string();
