...
impl Database {
    ...
    /// Get all posts from users the user is following, 25 at a time
    ///
    /// # Arguments
    /// * `page`
    /// * `user`
    pub async fn get_posts_by_following_paginated(
        ...
        let query: String = if (self.base.db.r#type == "sqlite") | (self.base.db.r#type == "mysql")
        {
            format!("SELECT * FROM \"xresponses\" WHERE \"context\" LIKE '%\"is_post\":true%' AND (\"author\" = ?{query_string}) ORDER BY \"timestamp\" DESC LIMIT 25 OFFSET {}", page * 25)
        } else {
            format!("SELECT * FROM \"xresponses\" WHERE \"context\" LIKE '%\"is_post\":true%' AND (\"author\" = $1{query_string}) ORDER BY \"timestamp\" DESC LIMIT 25 OFFSET {}", page * 25)
        };
    ...
    /// Get a [`Site`] from a database result
    pub async fn gimme_site(&self, res: HashMap<String, String>) -> Result<Site> {
        Ok(Site {
            id: res.get("id").unwrap().to_string(),
            ...
            edited: res.get("edited").unwrap().parse::<u128>().unwrap(),
            content: serde_json::from_str(res.get("content").unwrap()).unwrap(),
    ...
    /// # Arguments
    /// * `id`
    pub async fn get_site(&self, id: String) -> Result<Site> {
        // check in cache
        match self
            .base
            .cachedb
            .get(format!("rbeam.app.site:{}", id))
            .await
        {
            Some(c) => {
                match serde_json::from_str::<HashMap<String, String>>(c.as_str()) {
                    Ok(res) => {
                        return Ok(match self.gimme_site(res).await {
                            Ok(r) => r,
                            Err(e) => return Err(e),
                        })
                    }
                    Err(_) => {
                        ...
                        self.base
                            .cachedb
                            .remove(format!("rbeam.app.site:{}", id))
                            .await
                    }
                };
            }
        ...
        let query: String = if (self.base.db.r#type == "sqlite") | (self.base.db.r#type == "mysql")
        {
            "SELECT * FROM \"xsites\" WHERE \"id\" LIKE ?"
        } else {
            "SELECT * FROM \"xsites\" WHERE \"id\" LIKE $1"
        }
        .to_string();
        ...
        // return
        let site = match self.gimme_site(res).await {
            Ok(r) => r,
            Err(e) => return Err(e),
        };
        ...
        self.base
            .cachedb
            .set(
                format!("rbeam.app.site:{}", id),
                serde_json::to_string::<Site>(&site).unwrap(),
            )
            .await;
    ...
    /// Get an existing site (given its slug)
    ///
    /// # Arguments
    /// * `slug`
    pub async fn get_site_by_slug(&self, slug: String) -> Result<Site> {
        // check in cache
        match self
            .base
            .cachedb
            .get(format!("rbeam.app.site/slug:{}", slug))
            .await
        {
            Some(c) => {
                match serde_json::from_str::<HashMap<String, String>>(c.as_str()) {
                    ...
                    Err(_) => {
                        ...
                        self.base
                            .cachedb
                            .remove(format!("rbeam.app.site/slug:{}", slug))
                            .await
                    }
                };
            }
        ...
        self.base
            .cachedb
            .set(
                format!("rbeam.app.site/slug:{}", slug),
                serde_json::to_string::<Site>(&site).unwrap(),
            )
            .await;
    ...
    /// # Arguments
    /// * `author`
    pub async fn get_sites_by_author_paginated(
        &self,
        author: String,
        page: i32,
    ) -> Result<Vec<Site>> {
        ...
        let query: String = if (self.base.db.r#type == "sqlite") | (self.base.db.r#type == "mysql")
        {
            format!("SELECT * FROM \"xsites\" WHERE \"owner\" = ? ORDER BY \"edited\" DESC LIMIT 25 OFFSET {}", page * 25)
        } else {
            format!("SELECT * FROM \"xsites\" WHERE \"owner\" = $1 ORDER BY \"edited\" DESC LIMIT 25 OFFSET {}", page * 25)
        };
            ...
            .await
        {
            Ok(p) => {
                ...
                for row in p {
                    let res = self.base.textify_row(row, Vec::new()).0;
                    out.push(match self.gimme_site(res).await {
                        Ok(r) => r,
                        Err(e) => return Err(e),
                    });
                }
    ...
    pub async fn create_site(&self, props: SiteCreate, author: String) -> Result<Site> {
        // make sure site doesn't already exist
        if let Ok(_) = self.get_site_by_slug(props.slug.clone()).await {
        ...
        let query: String = if (self.base.db.r#type == "sqlite") | (self.base.db.r#type == "mysql")
        {
            "INSERT INTO \"xsites\" VALUES (?, ?, ?, ?, ?, ?, ?)"
        } else {
            "INSERT INTO \"xsites\" VALEUS ($1, $2, $3, $4, $5, $6, $7)"
        }
        .to_string();
        ...
        match sqlquery(&query)
            ...
            .bind::<&String>(&serde_json::to_string(&site.content).unwrap())
    ...
    pub async fn update_site_content(
        &self,
        id: String,
        content: crate::leafml::Plant,
        user: Profile,
    ) -> Result<()> {
        // make sure the site exists
        let site = match self.get_site(id.clone()).await {
            Ok(q) => q,
            Err(e) => return Err(e),
        };
        ...
        let query: String = if (self.base.db.r#type == "sqlite") | (self.base.db.r#type == "mysql")
        {
            "UPDATE \"xsites\" SET \"content\" = ?, \"edited\" = ? WHERE \"id\" = ?"
        } else {
            "UPDATE \"xsites\" SET (\"content\", \"edited\") = ($1, $2) WHERE \"id\" = $3"
        }
        .to_string();

        let c = &self.base.db.client;
        match sqlquery(&query)
            .bind::<&String>(&serde_json::to_string(&content).unwrap())
            .bind::<&String>(&utility::unix_epoch_timestamp().to_string())
            .bind::<&String>(&id)
            .execute(c)
            .await
        {
            Ok(_) => {
                ...
                self.base
                    .cachedb
                    .remove(format!("rbeam.app.site:{}:{}", site.owner, site.slug))
                    .await;
    ...
    /// * `user` - the user doing this
    pub async fn delete_site(&self, id: String, user: Profile) -> Result<()> {
        ...
        let query: String = if (self.base.db.r#type == "sqlite") | (self.base.db.r#type == "mysql")
        {
            "DELETE FROM \"xsites\" WHERE \"id\" = ?"
        } else {
            "DELETE FROM \"xsites\" WHERE \"id\" = $1"
        }
        .to_string();
